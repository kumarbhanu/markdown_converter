...
system_prompt = (
    ...
    "Return ONLY the code wrapped in triple backticks with 'html' language specifier. "
...
if input_text:
    ...
    # Extract code from answer using regex
    code_blocks = re.findall(r'```html(.*?)```', response["answer"], re.DOTALL)
    generated_code = code_blocks[0].strip() if code_blocks else response["answer"].strip()

    # Create two columns for display
    col1, col2 = st.columns(2)
    
    # Left column for generated code
    with col1:
        st.subheader("Generated Code")
        st.code(generated_code, language='html')
    
    # Right column for output preview
    with col2:
